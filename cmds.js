        ...
        onStart: async function ({ api, event, args, getLang }) {
                ...
                try {
                        ...
                        if (args[0]?.toLowerCase() === "info") {
                                ...
                                const { data } = await axios.get(`${baseURL}/api/cmdstore/info?name=${encodeURIComponent(cmdName)}&source=info`, axiosConfig);
                                if (!data?.success) {
                                        api.setMessageReaction("❌", event.messageID, () => {}, true);
                        ...
                        if (args[0]?.toLowerCase() === "rules") {
                                const { data } = await axios.get(`${baseURL}/api/cmdstore?type=rules`, axiosConfig);
                                if (!data?.success) {
                                        api.setMessageReaction("❌", event.messageID, () => {}, true);
